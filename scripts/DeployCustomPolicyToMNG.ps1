...
Param (
    ...
    [Parameter(Mandatory = $true)]
    [argumentcompleter( { (get-childitem "$PSScriptRoot/../esPoliciesDefinitionsCustom/*.json").name })]
    [string]$file,
...
$policyfile = "$PSScriptRoot/../esPoliciesDefinitionsCustom/$file"
...
If (Test-Path -Path $policyfile  ) {
    ...
    write-host "Error file not founct"
}
